...
func orient_projectile() -> void:
	...
	%Sprite.play(Global.get_direction_name(velocity))
...
func _on_body_entered(body: Node2D) -> void:
	...
	# makes sure to only hit player hurtboxes (areas, not bodies)
	if (not body.is_class("TileMapLayer") and body.get_collision_layer_value(2)):
		return
